...
class ReplayExecutionTests(unittest.TestCase):
    ...
    def test_provider_quality_fail_fast_stops_on_second_repeat_semantic_variance(self):
        telemetry = _MutableTelemetry()
        first_case = self.provider_quality_catalog.cases[0].case_id
        adapter = _ProviderQualityRecordedAdapter(
            self.provider_quality_catalog,
            self.claim_catalog,
            telemetry,
            semantic_repeat_variation_case=first_case,
        )
        ...
        self.assertEqual((first_case,), report.quality_outcome_variance_case_ids)

...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--mode", choices=("smoke", "final"), required=True)
    parser.add_argument(
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    repeats = 1 if args.mode == "smoke" else 3
    if args.mode == "smoke":
        provider_policy_catalog = select_provider_policy_cases(
            provider_policy_catalog,
            case_ids=(SMOKE_CASE_ID,),
        )
    planned_calls = repeats * len(provider_policy_catalog.cases)
    if planned_calls not in {1, 24}:

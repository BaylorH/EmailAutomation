...
_MISSING_FIELD_REQUEST_INTENT_RE = re.compile(
    r"(?:"
    r"^\s*(?:please\s+)?(?:confirm|provide|send|share|clarify|supply|forward)\b"
    r"|\b(?:can|could|would|will|may|might)\s+(?:i|you|we|they)\s+"
    r"(?:(?:please|also|kindly)\s+)*"
    r"(?:confirm|provide|send|share|clarify|supply|forward|tell)\b"
    r"|\b(?:could|would|do)\s+you\s+mind\s+(?:please\s+)?"
    r"(?:confirming|providing|sending|sharing|clarifying|supplying|forwarding|telling)\b"
    r"|\b(?:could|would)\s+it\s+be\s+possible\s+"
    r"(?:for\s+you\s+)?to\s+(?:please\s+)?"
    r"(?:confirm|provide|send|share|clarify|supply|forward|tell)\b"

...
Required semantic matching preserves evidence index, subject, predicate, value, actor role, polarity, modality, unit, effective date, and correction linkage, while treating numerically equal integer/float representations as equivalent. It intentionally ignores the exact contiguous `evidenceText` span and precise accepted confidence number: multiple validator-approved excerpts can support the same fact, and the prompt's `0.99` instruction conflicts with older accepted fixture confidence values between `0.95` and `0.98`. For the text-backed `remediation` and `correction` predicates, exact value text is also span-equivalent after deterministic validation: remediation must contain an explicit repair action, while correction must still satisfy prior-claim, speaker, subject, chronology, and old-value checks. The fixture's full accepted digests remain unchanged and continue to govern deterministic candidate validation.
